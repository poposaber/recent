...
import dinov2_features_25_dim as d2
...
def extract_features_to_h5(real_root, fake_root, output_h5, dataset_name="Dataset"):
    ...
    if len(all_videos) == 0:
        print(f"[{dataset_name}] No videos found. Skip writing {output_h5}.")
        return
    ...
    with h5py.File(output_h5, "w") as h5f:
        ...
        for idx in tqdm(range(0, len(all_videos), batch_size), desc=f"[{dataset_name}] Extracting features"):
            ...
            for j, (path, label, feat) in enumerate(zip(batch_paths, batch_labels, feats)):
                pos = idx + j
                path_ds[pos] = path
                label_ds[pos] = label
                feat_ds[pos, :] = feat
