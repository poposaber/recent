...
import dinov2_features_all as d2
...
def extract_features_to_h5(real_root, fake_root, output_h5, dataset_name="Dataset", dim=21):
    """
    ...
    with h5py.File(output_h5, "w") as h5f:
        ...
        feat_ds = h5f.create_dataset("features", (len(all_videos), dim), dtype="f")
        ...
        for idx in tqdm(range(0, len(all_videos), batch_size), desc=f"[{dataset_name}] Extracting features"):
            ...
            feats = d2.features_from_Z(Z, dim=dim).cpu().numpy()
...
if choice == "1" or choice == "3":
    extract_features_to_h5(train_real_root, train_fake_root, train_output_h5, dataset_name="TRAINING", dim=dim)

# Extract Testing Data
if choice == "2" or choice == "3":
    extract_features_to_h5(test_real_root, test_fake_root, test_output_h5, dataset_name="TESTING", dim=dim)
